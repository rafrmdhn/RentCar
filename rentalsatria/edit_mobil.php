<?php 
    ...
    if(isset($_GET['id'])){
        $view = "SELECT * FROM tbl_mobil WHERE id_mobil = '$_GET[id]'";
        $tampil = mysqli_query($conn, $view);
		$data = mysqli_fetch_array($tampil);
        if($data){
            $vid_mobil = $data['id_mobil'];
            $vid_type = $data['id_type'];
            $vnama_kendaraan = $data['nama_kendaraan'];
            $vno_plat = $data['no_plat'];
            $vwarna = $data['warna'];
            $vtahun = $data['tahun'];
            $vstatus_sewa = $data['status_sewa'];
            $vharga = $data['harga'];
            $vgambar = $data['gambar'];
        } else {
            echo "<script>
...
<body class="sb-nav-fixed">
    ...
    <div class="container-fluid px-4">
        <h1 class="mt-4">Tambah Data Product</h1>
                ...
                <div class="col-12 col-md-8">                    
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="id_type" required="">
                            ...
                            <?php
                                $query_type = mysqli_query($conn, "SELECT * FROM tbl_type");
                                while ($data = mysqli_fetch_assoc($query_type)) {?>
                                    <option value="<?=$data['id_type']?>" <?=@$vid_type == $data['id_type'] ? 'selected' : ''?>><?=$data['jenis']?></option>
                                <?php }?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">Nama Kendaraan</label>
                        <input type="hidden" value="<?=@$vid_mobil?>" class="form-control" name="id_mobil">
                        <input type="text" name="nama_kendaraan" class="form-control" placeholder="Nama Kendaraan" required="" value="<?=@$vnama_kendaraan?>">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">No Plat</label>
                        <input type="text" name="no_plat" class="form-control" placeholder="Nomor Plat" required="" value="<?=@$vno_plat?>">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">Warna</label>
                        <input type="text" name="warna" class="form-control" placeholder="Warna" required="" value="<?=@$vwarna?>">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">Tahun Kendaraan</label>
                        <input type="text" name="tahun" class="form-control" placeholder="Tahun Kendaraan" required="" value="<?=@$vtahun?>">
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlSelect1">Status Sewa</label>
                        <select class="form-control" name="status_sewa" required="">
                            <option>-- Pilih Status --</option>
                            <option value="0" <?=@$vstatus_sewa == '0' ? 'selected' : ''?>>Tersedia</option>
                            <option value="1" <?=@$vstatus_sewa == '1' ? 'selected' : ''?>>Tidak Tersedia</option>
                        </select>
                    </div>
                    ...
                    <div class="input-group mb-2">
                        ...
                        <input type="text" class="form-control" name="harga" placeholder="Harga Sewa" value="<?=@$vharga?>">
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlFile1">Gambar Product</label>
                        <input type="file" class="form-control-file" name="gambar">
                        <input type="hidden" name="gambar_lama" value="<?=@$vgambar?>">
                    </div>
...
<?php 
    ...
    if(isset($_GET['id'])) {                   
        ...
        if(isset($_POST['simpan'])){
            ...
            $query_update = "UPDATE tbl_mobil SET id_mobil=$id_mobil, id_type=$id_type,
                            nama_kendaraan='$nama_kendaraan', no_plat='$no_plat', warna='$warna',
                            harga=$harga, gambar='$gambar', tahun=$tahun,
                            status_sewa=$status_sewa WHERE id_mobil=$id";
